...
function DataTable({
  ...
  selectable = false,
  itemKey: propItemKey,
  updateSelection: propUpdateSelection = () => {},
}) {
  // headers가 있는지 체크하고 없으면 에러
  if (!headers || !headers.length) {
    throw new Error('<DataTable />에 헤더가 없습니다.')
  }
  ...
  const itemKey = propItemKey || headerKey[0]

  // 선택한 row의 itemKey를 담을 배열
  const [selection, setSelection] = useState(new Set())

  const updateSelection = (selectedItems) => {
    propUpdateSelection(selectedItems)
  }
  ...
  const onChangeSelect = (value) => {
    ...
    if (newSelection.has(value)) {
      // value가 있으면 삭제(checked가 false)
      newSelection.delete(value)
    } else {
      // value가 없으면 추가(checked가 true)
  ...
  const onChangeSelectAll = (e) => {
    if (e.target.checked) {
      ...
      const allCheckedSelection = new Set(
        // 활성화된 행의 배열을 순회하며 itemKey로 요소에 접근해 데이터 저장
